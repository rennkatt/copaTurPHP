 ...
 if($count > 0)
 {
  foreach($result as $row)
  {
   ...
   <div class="panel panel-default" style="margin-left:'.$marginleft.'px">
    <div class="panel-heading">By <b>'.$row["comment_sender_name"].'</b> on <i>'.$row["date"].'</i></div>
    <div class="panel-body">'.$row["comment"].'</div>
    <div class="panel-footer" align="right"><button type="button" class="btn btn-default reply" id="'.$row["comment_id"].'">Responder</button>
    </div>
   </div>
   ';
...
if(isset($_GET['delete'])){
    ...
    if($nivelLogado = 1){
        ...
        $seleciona = "SELECT * from tbl_comment WHERE comment_id= :id_delete";
        try{
          ...
          if($contar>0){
            ...
            // exclui o registo
            $seleciona = "DELETE from tbl_comment WHERE comment_id=:id_delete";
            try{
              $result = $connect->prepare($seleciona);
              $result->bindParam('id_delete',$id_delete, PDO::PARAM_INT);       
              $result->execute();
